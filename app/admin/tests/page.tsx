...
import { User as UserModel, Test } from '@/lib/models'
...
export default async function AdminTestsPage() {
  ...
  const mongoUser = await UserModel.findOne({ supabaseId: authUser.id }).lean()
        ...
        <div className="mb-8">
          ...
          {pending.length === 0 ? (
            ...
            <div className="space-y-3">
              {pending.map(test => {
                const teacher = test.teacher_id as any
                return (
                  <div key={String(test._id)} className="bg-white rounded-2xl border border-orange-100 shadow-sm p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 text-xs text-gray-500">
                          ...
                          <span>·</span>
                          <span className="flex items-center gap-0.5"><User className="w-3 h-3" /> {teacher?.full_name}</span>
                        </div>
                      </div>
                      <AdminTestActions testId={String(test._id)} />
                    </div>
                  </div>
                )
              })}
        ...
        <div>
          ...
          <div className="space-y-2">
            {approved.map(test => {
              const teacher = test.teacher_id as any
              return (
                <div key={String(test._id)} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{test.subject} · {teacher?.full_name}</p>
                  </div>
                  <AdminTestActions testId={String(test._id)} isApproved={true} />
                </div>
              )
            })}
          </div>
